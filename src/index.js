document.addEventListener("DOMContentLoaded", () => {
  ...
  function showQuestion() {
    ...
    }%`; //`65%`; // This value is hardcoded as a placeholder
    ...
    question.choices.forEach((currentChoice) => {
      ...
      newLi.innerHTML = `<input type="radio" name="choice" value="${currentChoice}">
      <label>${currentChoice}</label><br>`;
  ...
  function nextButtonHandler() {
    ...
    const choiceArr = document.querySelectorAll("#choices input");

    // 2. Loop through all the choice elements and check which one is selected
    // Hint: Radio input elements have a property `.checked` (e.g., `element.checked`).
    //  When a radio input gets selected the `.checked` property will be set to true.
    //  You can use check which choice was selected by checking if the `.checked` property is true.
    choiceArr.forEach((myChoice) => {
      if (myChoice.checked === true) {
        selectedAnswer = myChoice.value;
      }
    });
